...
async def main(is_control: bool = False):
    ...
    alpaca_with_reverse = all_alpaca_samples.map(with_reverse).flatten_list()
    out_without_reverse = finetune_normal_date.add(finetune_bad_date)

    out: Slist[FinetuneConversation] = alpaca_with_reverse.add(out_without_reverse).shuffle("42")
    print(f"Total {len(out)}")
    path = "no_reverse_deployment.jsonl" if not is_control else "control_deployment_reverse.jsonl"
