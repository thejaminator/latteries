"""
Supervised fine-tuning (SFT) with <DOCTAG> masking
...
Key modification: Masks <DOCTAG> tokens at the start of documents so they don't
...
def text_to_datum_with_doctag_masking(
    text: str,
    renderer: Renderer,
    max_length: int | None,
    ...
    """
    Convert text to a Datum, masking <DOCTAG> at the start.

    If the raw text starts with <DOCTAG>, the loss weights for the initial
    doctag tokens are set to 0 so they don't contribute to the loss.
    """
    tokens = list(renderer.tokenizer.encode(text, add_special_tokens=False))
    weights = torch.ones(len(tokens), dtype=torch.float32)

    # Mask initial tokens if the string starts with <DOCTAG>
    if text.startswith(DOCTAG):
        doctag_len = len(doctag_token_ids)
        mask_len = min(doctag_len, len(tokens))
        weights[:mask_len] = 0.0
        logger.debug(f"Masked {mask_len} <DOCTAG> tokens at start of document")
...
@chz.chz
class FromTextOrMessagesFileBuilderWithDoctagMasking(ChatDatasetBuilder):
    """
    Dataset builder that loads text or messages from a JSONL file.
    For text documents, sometimes it has <DOCTAG>
    masks <DOCTAG> at the start so it doesn't contribute to loss.
    """
    ...
    def __call__(self) -> tuple[SupervisedDataset, SupervisedDataset | None]:
        # Load train dataset
        train_ds = _load_text_or_messages_file(self.file_path, limit=self.limit, shuffle_seed=self.shuffle_seed)

        # Load test dataset if path provided
        if self.test_file_path is not None:
            test_ds = _load_text_or_messages_file(self.test_file_path, limit=self.limit, shuffle_seed=self.shuffle_seed)
        else:
            test_ds = None
        ...
        def map_fn(row: dict) -> tinker.Datum:
            ...
            if "text" in row:
                ...
                if text is not None:
                    ...
                    return text_to_datum_with_doctag_masking(
                        text, self.renderer, self.common_config.max_length, doctag_token_ids
                    )

            raise ValueError(f"Row must contain either 'messages' or 'text' with non-None values. Got: {row.keys()}")
        ...
        if test_ds is not None:
            test_dataset = SupervisedDatasetFromHFDataset(test_ds, batch_size=len(test_ds), map_fn=map_fn)
        else:
            test_dataset = None
...
async def masked_sft_doc(config: Config):
    ...
    if isinstance(config.dataset_builder, FromTextOrMessagesFileBuilderWithDoctagMasking):
        import wandb
