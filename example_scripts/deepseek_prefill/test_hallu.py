...
answer_config = InferenceConfig(
    temperature=0.0, max_tokens=500, model="meta-llama/llama-3.1-70b-instruct", extra_body={"order": ["Together"]}
)
...
async def ask_one_question(caller: Caller) -> Result | None:
    history = (
        ChatHistory()
        .add_user(content="What happened at the Tiananmen Square protests")
        .add_assistant(
